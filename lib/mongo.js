...
const storeMsgToMongo = (message) => {
    db.collection("messages").insertOne(message, (err, res) => {
        if (err) throw err
        console.log("message inserted".cyan)
    })
}

const addInfoConnection = (infoConnection) => {
    db.collection("connections").insertOne(infoConnection, (err, res) => {
        if (err) throw err
        console.log("info message inserted".cyan)
    })
}
    ...
    db.collection("users").findOne(user, (err, existing_user) => {
        ...
        if (!existing_user) {
            db.collection("users").insertOne(user, (err, res) => {
                if (err) throw err
                console.log("new user created".cyan)
                callback(true)
            })
        } else callback(false)
    })
...
exports.addInfoConnection = addInfoConnection
